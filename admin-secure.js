// Secure Admin System - Uses Supabase Edge Functions for Authentication
// No sensitive credentials stored in client code!
// All admin operations go through secure Edge Functions

// ========== CONFIGURATION ==========
// This file now uses window.SUPABASE_SECURE from supabase-secure-config.js
// Make sure to include supabase-secure-config.js before this file

// Use STORAGE_KEYS from supabase-secure-config.js (already defined there)
// No need to redeclare - it's available globally
...
async function handleLogin() {
    ...
    try {
        ...
        const result = await window.SUPABASE_SECURE.auth.login(username, password);
        
        if (result.success) {


            // User info is already saved by SecureAuth.login()
            // Redirect based on user role
            const userRole = result.role || 'admin';
            if (userRole === 'super_admin') {
                window.location.href = 'admin-dashboard.html';
            } else {
                // Regular admin - redirect to first allowed page
                window.location.href = 'admin-ramadan.html';
            }
        } else {
            // Show error
            errorMessage.textContent = result.message || 'اسم المستخدم أو كلمة المرور غير صحيحة';
...
async function adminLogout() {
    try {
        // Logout via secure Edge Function
        await window.SUPABASE_SECURE.auth.logout();
    } catch (error) {
...
// ========== USER MANAGEMENT (Now handled by Edge Functions) ==========
// Note: User management is now handled securely via Edge Functions
// Use window.SUPABASE_SECURE.api.adminUsers for all user operations

/**
 * Legacy function - kept for compatibility
 * @deprecated Use window.SUPABASE_SECURE.api.adminUsers instead
 */
